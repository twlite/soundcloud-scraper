...
class Client {
     ...
     */
    async apiVersion(force = false) {
        const existing = Store.get("SOUNDCLOUD_API_VERSION");
        if (existing && !force) return existing;

        try {
            const version = await Util.parseHTML(`${Constants.SOUNDCLOUD_BASE_URL}${Constants.SOUNDCLOUD_API_VERSION}`);
            return Store.set("SOUNDCLOUD_API_VERSION", version);
        } catch {
            return null;
        }
    }
     ...
     */
    async getSongInfo(url, options = { fetchEmbed: false, fetchComments: false, fetchStreamURL: false, requestOptions: {} }) {
        if (typeof url !== "string") throw new TypeError(`URL type must be a string, received "${typeof url}"!`);
        if (!Util.validateURL(url, "track")) throw new TypeError("Invalid song url!");
        const raw = await Util.parseHTML(url, options.requestOptions || {});
        if (!raw) throw new Error("Couldn't parse html!");
     ...
     */
    async getPlaylist(url, options = { fetchEmbed: false }) {
        if (!url || typeof url !== "string") throw new Error(`URL must be a string, received "${typeof url}"!`);
        if (!Util.validateURL(url, "playlist")) throw new TypeError("Invalid url!");
    ...
    async search(query, type = "all") {
        if (!query || typeof query !== "string") throw new Error(`Expected search query to be a string, received "${typeof query}"!`);
    ...
    async getUser(username) {
        if (!username || typeof username !== "string") throw new Error(`Expected username to be a string, received "${typeof username}"!`);
        const BASE_URL = Constants.SOUNDCLOUD_BASE_URL;
        if (!username.includes(BASE_URL)) username = `${BASE_URL}/${username}`;
        if (!Util.validateURL(username, "artist")) throw new Error("Invalid artist url");

        const html = await Util.parseHTML(username);
        const URNSearch = /soundcloud:\/\/users:(?<urn>\d+)/.exec(html);
        if (!URNSearch || !URNSearch.groups.urn) throw new Error("User not found!");
